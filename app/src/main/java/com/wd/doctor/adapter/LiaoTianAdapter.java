...
public class LiaoTianAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public LiaoTianAdapter(Context context, List<ChaXinXiBean.ResultBean> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == ONE) {
            Log.d("MyAdapter", "条目一");
            View view = View.inflate(context, R.layout.liaotianleft_adapter, null);
            return new LiaoLeft(view);
        } else if (viewType == TWO) {
            Log.d("MyAdapter", "条目二");
            View view1 = View.inflate(context, R.layout.liaotianright_adapter, null);
